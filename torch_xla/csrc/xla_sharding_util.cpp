...
// Convert tile assignment list (up to 5-dimension) to array.
xla::Array<int64_t> TileListToArray(const py::list& tile_list) {
  auto dims = TileAssignmentDimensions(tile_list);
  xla::Array<int64_t> tile_array(dims);
  switch (dims.size()) {
    case 1:
      tile_array.Each([&](absl::Span<const int64_t> indices, int64_t* v) {
        *v = tile_list[indices[0]].cast<int64_t>();
      });
      break;
    case 2:
      tile_array.Each([&](absl::Span<const int64_t> indices, int64_t* v) {
        auto r = tile_list[indices[0]].cast<py::list>();
...
std::vector<at::Tensor> ShardingUtil::ShardTensor(
  ...
  TF_VLOG(INFO) << "ShardTensor with sharding type(" << sharding.type()
                << ")..." << std::endl;
  auto device_index = build_index_map(devices);
  ...
  if (sharding.type() == xla::OpSharding::REPLICATED) {
    ...
    auto tile_dimensions = sharding.tile_assignment_dimensions();
    std::vector<int64_t> tile_shape;
    tile_shape.reserve(tile_dimensions.size());
    for (int i = 0; i < tile_dimensions.size(); ++i) {
      // Skip the last replication dimension
      if (!(sharding.replicate_on_last_tile_dim() &&
            (i == tile_dimensions.size() - 1))) {
        tile_shape.push_back(tile_dimensions[i]);
      }
    }
